...
function armAbandonmentBackstop(body, controller) {
  if (cleanups.has(controller))
    registry == null ? void 0 : registry.register(body, controller, controller);
}
function releaseRequestSignal(controller) {
  const cleanup = cleanups.get(controller);
  if (cleanup) {
    cleanups.delete(controller);
    registry == null ? void 0 : registry.unregister(controller);
    cleanup();
  }
}
...
async function checkCredentialsFileSafety(path2, onWarn = (m) => console.warn(`anthropic-sdk: ${m}`)) {
  if (typeof process === "undefined" || process.platform === "win32")
    return;
  const fs = await import("./node_fs-KP4XMRG4.js");
  let resolved = path2;
  let st;
  try {
...
async function writeCredentialsFileAtomic(targetPath, data) {
  const fs = await import("./node_fs-KP4XMRG4.js");
  const path2 = await import("./node_path-P3ZF5MV7.js");
...
async function _EnvironmentWorker_handleItem2(work, environmentKey, externalSignal) {
  ...
  const agentToolset = await import("./node.browser-YTOJ5DRU.js");
    ...
    while (index < fallbacks.length - 1) {
      ...
      res = await next({
        ...
        body: JSON.stringify({
          ...
          ...((_d = message.stop_details) == null ? void 0 : _d.fallback_credit_token) ? { fallback_credit_token: creditTokenParam(message.stop_details.fallback_credit_token) } : void 0
        })
      });
    }
